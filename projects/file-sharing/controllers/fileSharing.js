...
const generateLink = async (req, res) => {
  // "http://localhost:10000/files/download/:uuid"
  const uuid = req.params.uuid;
  // const file = await FileSharingModel.findOne({_id: uuid});
  const file = await FileSharingModel.findById(uuid);
  if (!file) {
    // Invalid file id
...
const downloadFile = async (req, res) => {
  ...
  res.download(file.path,"abc.json");
};
...
const sendFileLinkOnEmail = async (req, res) => {
  ...
  await FileSharingModel.updateOne(
    { _id: file._id },
